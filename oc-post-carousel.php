...
* Description: This plugin allows create Post Slider Carousel & Custom Post Grids plugin.
* Version: 1.0
* Copyright: 2020
* Text Domain: post-slider-carousel-custom-post-grids
...
if (!defined('PSCCPG_PLUGIN_NAME')) {
    define('PSCCPG_PLUGIN_NAME', 'Post Slider Carousel & Custom Post Grids');
}
...
if (!defined('PSCCPG_BASE_NAME')) {
    define('PSCCPG_BASE_NAME', plugin_basename(PSCCPG_PLUGIN_FILE));
}
if (!defined('PSCCPG_DOMAIN')) {
    define('PSCCPG_DOMAIN', 'post-slider-carousel-custom-post-grids');
}
...
if (!class_exists('PSCCPG')) {
    ...
    class PSCCPG {
        ...
        function PSCCPG_plugin_row_meta( $links, $file ) {
            if ( PSCCPG_BASE_NAME === $file ) {
                $row_meta = array(
                  'rating'    =>  '<a href="https://www.xeeshop.com/post-slider-carousel-custom-post-grids/" target="_blank">Documentation</a> | <a href="https://www.xeeshop.com/support-us/?utm_source=aj_plugin&utm_medium=plugin_support&utm_campaign=aj_support&utm_content=aj_wordpress" target="_blank">Support</a> | <a href="https://wordpress.org/support/plugin/post-slider-by-oc/reviews/?filter=5" target="_blank"><img src="'.PSCCPG_PLUGIN_DIR.'/assets/images/star.png" class="ocpc_rating_div"></a>',
                );
    ...
    register_activation_hook( PSCCPG_PLUGIN_FILE, 'PSCCPG_plugin_activation_func' );
}


add_action( 'plugins_loaded', 'PSCCPG_load_textdomain' );
function PSCCPG_load_textdomain() {
    load_plugin_textdomain( 'post-slider-carousel-custom-post-grids', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' ); 
}
function PSCCPG_load_my_own_textdomain( $mofile, $domain ) {
    if ( 'post-slider-carousel-custom-post-grids' === $domain && false !== strpos( $mofile, WP_LANG_DIR . '/plugins/' ) ) {
        $locale = apply_filters( 'plugin_locale', determine_locale(), $domain );
        $mofile = WP_PLUGIN_DIR . '/' . dirname( plugin_basename( __FILE__ ) ) . '/languages/' . $domain . '-' . $locale . '.mo';
    }
    return $mofile;
}
add_filter( 'load_textdomain_mofile', 'PSCCPG_load_my_own_textdomain', 10, 2 );